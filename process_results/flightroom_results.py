...
#%% Load trajectories
path_file = 'transforms_local_position.json'

r = 0.2
with open(path_file, 'r') as f:
    meta = json.load(f)

trajectories = [element["position"] for element in meta["6D_pose"]]
...
fig, ax = plt.subplots(1, figsize=(5, 5), dpi=1000)
ax.plot(np.linspace(0., 1., len(min_dists), endpoint=True)*30, min_dists)
ax.axhline(y = 0., color = 'r')#, alpha=0.3) 
# ax.set_xlabel('Time (s)')
# ax.set_ylabel('Distance to Closest Obstacle (m)')
