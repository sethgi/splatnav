...
#Import utilies
from nerfnav.nav import NerfNav, vec_to_rot_matrix
from nerfnav.nerf import NeRFWrapper
...
def simulate(planner_cfg, density_fn, iteration):
    ...
    # Initialize Planner
    traj = NerfNav(start_state, end_state, planner_cfg, density_fn)
...
# Run
T_final = 20.                # Final time of simulation
...
for scene_name in ['stonehenge', 'statues', 'flight', 'old_union']:
    ...
    if scene_name == 'old_union':
        ...
        path_to_config = 'outputs/old_union2/nerfacto/2024-09-12_203602/config.yml' # points to where the nerf params are stored
        ...
        path_to_config = 'outputs/stonehenge/nerfacto/2024-12-04_153827/config.yml'
        ...
        path_to_config = 'outputs/statues/nerfacto/2024-09-12_204832/config.yml'
        ...
        path_to_config = 'outputs/flight/nerfacto/2024-09-14_083406/config.yml'
    ...
    for trial, (start, goal) in enumerate(zip(x0, xf)):
        ...
        start_state = torch.cat( [x[:3], init_rates, start_R.reshape(-1), init_rates], dim=0).to(device)
        end_state   = torch.cat( [goal[:3],   init_rates, end_R.reshape(-1), init_rates], dim=0 ).to(device)
        ...
        output = simulate(planner_cfg, nerf.get_density, trial)
